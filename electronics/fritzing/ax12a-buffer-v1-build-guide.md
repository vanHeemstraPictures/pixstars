...
> IMPORTANT -- ROBOTIS U2D2 external-reader path (bench-proven). The ROBOTIS U2D2 is IN HAND (Reichelt; see `HARDWARE_INVENTORY.md`). The user-updated `electronics/fritzing/ax-12a-bench-test.svg` and the now-verified `electronics/fritzing/ax-12a-u2d2-external-read.svg` together show the external-reader wiring in the proven pin order that produced a successful Dynamixel Wizard 2.0 scan (Protocol 1.0, 1000000 bps, ID 1, model AX-12A). The **U2D2 has exactly one 3-pin TTL socket** (plus one 4-pin RS-485 socket, one UART header, and USB-C); the TTL socket is the sole AX-12A comm path. The two equivalent 3-pin sockets are on the AX-12A servo itself (daisy-chain), not on the U2D2. Pattern A in `ax-12a-u2d2-external-read.svg` uses the AX-12A's second socket for the U2D2 comm link (shared 12V motor rail); Pattern B is a breadboard 3-way junction fallback. In both patterns the AX-12A pigtail pin order matches this guide (Pin 1 = GND black, Pin 2 = VCC red, Pin 3 = DATA yellow). **The physical breadboard's default state remains the ESP32-direct data path** described in section 3 of this guide; the U2D2 is wired in only for the duration of the read-only external Protocol 1.0 servo-side check (Status Return Level, angle limits, ID, baud, return delay) with Dynamixel Wizard 2.0. During that read the ESP32 TX/RX/DIR wires and the 74HCT245 B1/B2 bridge must be isolated from the AX-12A DATA line so the two bus masters do not fight, and the +5V cave rail is never shared with the U2D2 (the U2D2 draws its logic supply from USB-C; only the +12V motor rail is shared). The Connection table in section 3 and the Verification checklist in section 6 describe the ESP32-direct default state and remain unchanged by this note. The authoritative bench sequence for the external U2D2 read lives in the "Run the U2D2 arrival-day readiness pass" task ([note 581d6b5b](intent://local/task/581d6b5b-91a4-45b4-8aae-554ce590b9e1)) and its downstream "Run the read-only external Status Return Level check" task -- do not derive an ad-hoc procedure from this build guide.
