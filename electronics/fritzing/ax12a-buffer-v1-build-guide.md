...
> IMPORTANT -- ROBOTIS U2D2 external-reader path (SVG-only today). The ROBOTIS U2D2 is IN HAND (Reichelt; see `HARDWARE_INVENTORY.md`). The user-updated `electronics/fritzing/ax-12a-bench-test.svg` now shows the intended external-reader wiring with the U2D2 in-line on the AX-12A 3-pin bus (shared 12V rail; TTL DATA passthrough on the U2D2's two equivalent 3-pin sockets). **The physical breadboard today still has the ESP32-direct data path** described in section 3 of this guide -- the U2D2 rewiring shown in the SVG has NOT been applied to hardware yet. The Connection table in section 3 and the Verification checklist in section 6 therefore still describe the current physical bench and remain unchanged by this note. The U2D2 path is executed only when running the read-only external Protocol 1.0 servo-side check (Status Return Level, angle limits, ID, baud, return delay) with Dynamixel Wizard 2.0; the ESP32 TX/RX/DIR wires must be disconnected from the AX-12A DATA line for the duration of that external read so the two bus masters do not fight. The authoritative bench sequence for the external U2D2 read lives in the "Run the U2D2 arrival-day readiness pass" task ([note 581d6b5b](intent://local/task/581d6b5b-91a4-45b4-8aae-554ce590b9e1)) and its downstream "Run the read-only external Status Return Level check" task -- do not derive an ad-hoc procedure from this build guide.
